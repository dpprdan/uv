...
impl ConflictingGroupList {
    ...
    pub fn none() -> ConflictingGroupList {
...
impl TryFrom<Vec<ConflictingGroup>> for ConflictingGroups {
    ...
    fn try_from(groups: Vec<ConflictingGroup>) -> Result<ConflictingGroups, ConflictingGroupError> {
        if groups.len() < 2 {
            return Err(ConflictingGroupError::NotEnoughGroups { len: groups.len() });
        }
...
pub enum ConflictingGroupError {
    /// An error for when there are too few conflicting groups.
    ///
    /// (A singleton or empty set of groups doesn't make sense.)
    #[error("Each set of conflicting groups must have at least 2 entries, but found only {len}")]
    NotEnoughGroups { len: usize },
}
