...
impl<'d> Forker<'d> {
    ...
    /// Returns the underlying package and extra name corresponding to the
    /// dependency represented by this `Forker`. If the underlying dependency
    /// lacks a name *or* an extra, then this returns `None`.
    #[allow(dead_code)] // will be used later
    fn package_extra(&self) -> Option<(&PackageName, &ExtraName)> {
        match (self.package.name_no_root(), self.package.extra()) {
            (Some(package), Some(extra)) => Some((package, extra)),
            _ => None,
        }
    }
}
